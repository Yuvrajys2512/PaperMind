"""
pdf_parser_v3.py — robust PDF text extractor for research papers.
...
Public API (same signature as pdf_parser.py so it's a drop-in replacement):
...
def _chars_to_line_text(chars: list) -> str:
...
def _process_single_column(chars: list) -> str:
    ...
    for _, line_chars in lines:
        text = _chars_to_line_text(line_chars)
        if text.strip():
...
def _process_two_column(chars: list, col_boundary: float,
    ...
    if not col_lines:
        ...
        return "\n".join(
            _chars_to_line_text(c).strip() for _, c in all_lines
            if _chars_to_line_text(c).strip()
        )
...
if __name__ == "__main__":
    ...
    # Also write full output to a file for inspection
    out_path = os.path.splitext(pdf_path)[0] + "_v3_output.txt"
