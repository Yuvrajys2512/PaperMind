...
const CMD_LIST = [
  ...
  { id: 'audit',      label: 'Audit claims vs evidence',   icon: '◎' },
  { id: 'review',     label: 'Review weaknesses',          icon: '⚑' },
...
<body>
  ...
  const handleCommand = (cmd) => {
    ...
    if (cmd === 'audit')     setShowAudit(true)
    if (cmd === 'review')    setShowReview(true)
  ...
  return (
    <div className="min-h-screen cosmic-bg flex flex-col" style={{ position: 'relative' }}>
      ...
      {showAudit     && <AuditPanel     paperId={paper.paper_id} onClose={() => setShowAudit(false)} />}
      {showReview    && <ReviewPanel    paperId={paper.paper_id} onClose={() => setShowReview(false)} />}
      ...
      <header className="sticky top-0 z-50 backdrop-blur-2xl border-b px-8 py-4"
        ...
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            ...
            {/* Audit — claim → evidence grounding */}
            <button onClick={() => setShowAudit(a => !a)}
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg transition-all"
              style={showAudit
                ? { background: 'rgba(0,245,255,0.1)', border: '1px solid rgba(0,245,255,0.25)', color: 'rgba(0,245,255,0.9)' }
                : { background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)', color: 'rgba(107,114,128,1)' }}
              title="Check the paper's claims against its own evidence">
              <span className="text-[11px]">◎</span>
              <span className="text-[9px] uppercase tracking-wider" style={{ fontFamily: 'var(--font-mono)' }}>Audit</span>
            </button>

            {/* Review — methodological weakness audit vs venue norms */}
            <button onClick={() => setShowReview(a => !a)}
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg transition-all"
              style={showReview
                ? { background: 'rgba(167,139,250,0.1)', border: '1px solid rgba(167,139,250,0.25)', color: 'rgba(167,139,250,0.9)' }
                : { background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)', color: 'rgba(107,114,128,1)' }}
              title="Critique the paper against venue norms — baselines, ablations, error bars, threats to validity">
              <span className="text-[11px]">⚑</span>
              <span className="text-[9px] uppercase tracking-wider" style={{ fontFamily: 'var(--font-mono)' }}>Review</span>
            </button>
