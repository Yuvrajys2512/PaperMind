...
/* ── METRIC RING ─────────────────────────────────────────────────── */
// Plain-language explanation for each metric, surfaced on hover. Keyed by the
// lowercased label so both the live and replayed renders share one source of
// truth. Mirrors the math in ingestion/evaluator.py.
...
export function MetricRing({ label, value, isPercentage = false, accent = 'cyan', tooltip = null }) {
  ...
  const strokeColor = accent === 'cyan' ? '#00f5ff' : accent === 'violet' ? '#a78bfa' : accent === 'amber' ? '#fbbf24' : accent === 'emerald' ? '#34d399' : accent === 'rose' ? '#fb7185' : '#60a5fa'
